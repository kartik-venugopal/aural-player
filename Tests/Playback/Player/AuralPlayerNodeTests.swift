...
class AuralPlayerNodeTests: AuralTestCase {
    ...
    private func doScheduleSegmentWithSession(_ trackDuration: Double, _ sampleRate: Double, _ startTime: Double, _ endTime: Double?,
        ...
        guard let theSegment = segment, let playbackCtx = track.playbackContext else {
            
            XCTFail("Segment and track playback context were expected to be non-nil.")
            return
        }
        ...
        let totalFrameCount = playbackCtx.frameCount
        ...
        XCTAssertEqual(theSegment.playingFile.url, track.playbackContext?.file)
    }

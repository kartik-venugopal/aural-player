//
//  FlatPlaylistTests+MoveTracksToBottom.swift
//  Tests
//
...
class FlatPlaylistTests_MoveTracksToBottom: FlatPlaylistTestCase {
    
    func test_emptyPlaylist() {
        
        assertEmptyPlaylist()
        
        var result = playlist.moveTracksToBottom(IndexSet([0, 1]))
        XCTAssertTrue(result.playlistType == .tracks)
        XCTAssertTrue(result.results.isEmpty)
        
        result = playlist.moveTracksToBottom(IndexSet([]))
        XCTAssertTrue(result.playlistType == .tracks)
        XCTAssertTrue(result.results.isEmpty)
    }
    ...
    private func doTest(playlistSize: Int, movedTrackIndices: Set<Int>) {
        ...
        let sortedMoveIndices = movedTrackIndices.sorted(by: Int.descendingIntComparator)
        let lastPlaylistIndex = playlistSize - 1
        
        for (destinationIndex, sourceIndex) in sortedMoveIndices.enumerated().map({(lastPlaylistIndex - $0, $1)}).filter({$0.0 != $0.1}) {
            sourceDestinationMap[sourceIndex] = destinationIndex
        }
        
        let results = playlist.moveTracksToBottom(IndexSet(movedTrackIndices))

...
class LastFM_WSClient: LastFM_WSClientProtocol {
    ...
    private var lastFMPreferences: LastFMPreferences {
    ...
    func retryFailedScrobbleAttempts() {
        ...
        let prefs = preferences.metadataPreferences.lastFM
        guard prefs.enableScrobbling.value, let sessionKey = prefs.sessionKey.value else {return}
    ...
    private func favoriteAdded(favorite: Favorite) {
        ...
        guard lastFMPreferences.enableLoveUnlove.value else {
            // TODO: Can we love/unlove a non-track ???
            // TODO: Log an error if Fav is not a track ???
            return
        }
        
        guard let sessionKey = lastFMPreferences.sessionKey.value else {
            
            NSLog("Cannot love track '\(favorite.name)' on Last.fm because no session key is available.")
            return
        }
        
        // TODO: Can we love/unlove a non-track ???
        
        guard let favTrack = favorite as? FavoriteTrack else {
            
            NSLog("Cannot love track '\(favorite.name)' on Last.fm because it does not have both title and artist metadata.")
            return
        }
        
        DispatchQueue.global(qos: .background).async {
            self.loveTrack(track: favTrack.track, usingSessionKey: sessionKey)
        }
    }

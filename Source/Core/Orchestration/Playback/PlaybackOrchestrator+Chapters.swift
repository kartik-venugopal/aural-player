//
// PlaybackOrchestrator+Chapters.swift
// Aural
// 
...
extension PlaybackOrchestrator {
    ...
    func playChapter(index: Int) {
        ...
        seekTo(position: startTime + (startTime > 0 ? Self.chapterPlaybackStartTimeMargin : 0))
        
        // Resume playback if paused
        resumeAfterSeekIfPaused()
        
        ui?.chapterChanged(state: state, position: playbackPosition!, loop: playbackLoop, loopState: playbackLoopState)
    }
    
    func previousChapter() {
        
        if let index = previousChapterIndex {
            playChapter(index: index)
        }
    }
    ...
    func toggleChapterLoop() -> Bool {
        ...
        if !chapterLoopExists {
            ...
            player.defineLoop(startPosition: startTime, endPosition: endTime, isChapterLoop: true)
            return true
    ...
    var currentChapterIndex: Int? {
        ...
        let seekTime = player.playerPosition
        var first = 0
    ...
    var previousChapterIndex: Int? {
        ...
        if player.playerPosition < curChapter.startTime {

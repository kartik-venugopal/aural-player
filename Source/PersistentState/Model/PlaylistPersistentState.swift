...
class PlaylistPersistentState: PersistentStateProtocol {
    ...
    required init?(_ map: NSDictionary) {
        ...
        if let groupingPlaylistsMap = map["groupingPlaylists"] as? NSDictionary {
            ...
            if let artistsPlaylist = groupingPlaylistsMap.objectValue(forKey: "artists", ofType: GroupingPlaylistPersistentState.self) {
                self.groupingPlaylists?["artists"] = artistsPlaylist
            }
            
            if let albumsPlaylist = groupingPlaylistsMap.objectValue(forKey: "albums", ofType: GroupingPlaylistPersistentState.self) {
                self.groupingPlaylists?["albums"] = albumsPlaylist
            }
            
            if let genresPlaylist = groupingPlaylistsMap.objectValue(forKey: "genres", ofType: GroupingPlaylistPersistentState.self) {
...
class GroupingPlaylistPersistentState: PersistentStateProtocol {
    ...
    required init?(_ map: NSDictionary) {
        ...
        self.type = type
        self.groups = map.arrayValue(forKey: "groups", ofType: GroupPersistentState.self)
    }
}

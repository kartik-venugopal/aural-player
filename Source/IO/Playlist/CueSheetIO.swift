...
class CueSheetIO: PlaylistIOProtocol {
    ...
    static func loadPlaylist(_ playlistFile: URL) -> SavedPlaylist? {
        ...
        while cursor < lines.count {
            ...
            if line.starts(with: prefix_file), let file = readFile(0) {
    ...
    private static func readFile(_ rootIndentLevel: Int) -> (file: URL, chapters: [Chapter])? {
        
        // FILE "201-va_-_trance_the_ultimate_collection_best_of_2007.mp3" MP3
        ...
        for index in 0..<chapterTitlesAndStartTimes.count {
            ...
            chapters.append(Chapter(title ?? String(format: "Chapter %d", index + 1), correctedStart, correctedEnd, correctedDuration))
        }
    ...
    private static func readTitleOrPerformer() -> String? {
        ...
        let tokens = line.components(separatedBy: " ")
        return tokens.count >= 2 ? tokens[1].replacingOccurrences(of: "\"", with: "") : nil
    }

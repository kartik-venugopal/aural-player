...
class MetadataReader {
    ...
    static func loadAllMetadata(_ track: Track) {
        ...
        let metadataList = track.avAsset!.metadata
        
        let formats = track.avAsset!.availableMetadataFormats
        
//        print("\nFor track:", track.conciseDisplayName)
//        print(formats)
        
        for item in metadataList {
            
//            print("keyspace", item.keySpace)
            
            let val = item.stringValue
            
            if let key = item.commonKey {
                
//                print(String(format: "Common: %@ = %@", key, val ?? "nil"))
                ...
                if let key = item.key as? String {
                    ...
                    //                print(String(format: "ID3: %@ (%@) = %@", key, ID3Spec.forKey(key) ?? "???", val ?? "nil"))
                    
                    if (!Utils.isStringEmpty(val)) {
//                        let entry = MetadataEntry(.id3, key, val!)
//                        track.metadata[key] = entry
                    }
...
class ID3Spec {
    ...
    static var map: [String: String] = initMap()
